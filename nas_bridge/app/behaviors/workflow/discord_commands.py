"""Workflow-specific Discord command surface."""
...
class WorkflowDiscordCommandProvider:
    def __init__(
        self,
        *,
    ...
    def register_commands(self, tree: app_commands.CommandTree) -> None:
        ...
        @app_commands.describe(
            query="Describe the project or folder to resume",
            preset="Optional execution profile from registered YAML",
        )
        ...
        async def verify_latest(interaction: discord.Interaction) -> None:
            ...
            if run is None:
                await interaction.followup.send(
                    "No verification run has been recorded for this session yet.",
                    ephemeral=True,
                )
                return
            artifact_lines = "\n".join(
